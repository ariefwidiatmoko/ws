...
@section('navmenu')
  ...
  <a href="{{ route('subjects.index') }}">Subjects</a> <i class="fa fa-angle-right fa-fw" style="color: #3c8dbc;"></i>
  <a class="active" style="color: grey;">@yield('title')</a>
@endsection

@section('button')
  <a href="{{ route('subjects.index') }}" class="btn btn-xs btn-default">Back</a>
@endsection
...
<div class="content" id="main-content" style="margin-top: -20px; margin-left: 1px;">
  ...
  <div class="box" style="margin-top: -20px;">
    ...
    <div class="box-body" style="margin-left: 15px; margin-right: 15px;">
      <form enctype="multipart/form-data" role="form" action="{{ route('subjects.update', $subject->id) }}" method="POST">
          {{ method_field('PUT') }}
          {{ csrf_field() }}
        @include('settings.subjects._form_edit')
        <div class="">
          <br>
          <!-- Submit Form Button -->
          {!! Form::submit('Save', ['class' => 'btn btn-xs btn-success']) !!}
          <!-- Back Button -->
          <a href="{{ route('subjects.index') }}" type="button" class="btn btn-default btn-xs">Cancel</a>
        </div>
      </form>
    </div>
    <!-- /.box-body -->
