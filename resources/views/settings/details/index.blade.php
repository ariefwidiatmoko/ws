...
@section('button')
  ...
  <a href="{{ route('details.create') }}" class="btn btn-xs btn-success"><i class="fa fa-plus fa-fw"></i> New Detail Score</a>
          ...
          <table id="inlist" class="table table-hover">
            ...
            @forelse ($result as $index => $item)
              <tr>
                ...
                <td style="text-align: center;">{{ ucfirst($item->detailname) }}</td>
                    ...
                    <div class="btn-group" role="group">
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">
                        @can ('edit_positions', $item)
                          <a href="{{ route('details.edit', $item->id) }}" class="btn btn-xs btn-info">Edit</a>
                        @endcan
                      </div>
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">
                        @can ('delete_positions', $item)
                          {!! Form::open( ['method' => 'delete', 'url' => route('details.destroy', $item->id), 'onSubmit' => 'return confirm("Are yous sure wanted to delete ' . $item->detailname . ' ?")']) !!}
