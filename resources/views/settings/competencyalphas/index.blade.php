...
@section('title', 'Competency Scale')
...
@section('button')
  ...
  <button type="button" id="toggle-btn" class="btn btn-xs btn-success"><i class="fa fa-plus fa-fw"></i> Add Competency Scale</button>
...
<div class="box box-primary collapsed-box" style="margin-top: 20px;">
  ...
  <div class="box" style="margin-top: -20px;">
    ...
    <div class="toggle col-xs-12" style="margin-left: 10px; margin-bottom: 15px;">
      <form enctype="multipart/form-data" role="form" action="{{route('competencyalphas.store')}}" method="POST">
        {{ csrf_field() }}
        <div class="row">
          <div class="col-xs-2">
        ...
        <div class="col-sm-12">
          <table id="inlist" class="table table-hover">
            ...
            @forelse ($result as $index => $item)
                  ...
                  <div class="row">
                    <div class="btn-group" role="group">
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">
                        ...
                        <a class="column-competency-modal btn btn-xs btn-info" data-id="{{$item->id}}" data-score1="{{$item->score[0]}}" data-score2="{{$item->score[1]}}" data-alphabet="{{$item->alphabet}}" data-description="{{$item->description}}">Edit</a>

                      </div>
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">

                        {!! Form::open( ['method' => 'delete', 'url' => route('competencyalphas.destroy', $item->id), 'onSubmit' => 'return confirm("Are yous sure wanted to delete ' . $item->alphabet . ' ?")']) !!}
              ...
              @empty
              <tr>
                <td colspan="8">No Competency Scale</td>
              </tr>
              @endforelse
              </tbody>
            </table>
...
@include('settings.competencyalphas._modal_columncompetency')
@endsection
...
@section('scripts')
    ...
    <script>
        $(document).ready(function(){
            ...
            {{--hide Add Competency Scale--}}
            $("div.toggle").hide();
            {{--Togle Competency Scale--}}
        ...
        {{--Edit Competency Scale--}}
        $(document).on('click', '.column-competency-modal', function() {
            $('.modal-title').text('Edit Competency Scale');
        ...
        $('.detail').on('click', '.edit', function() {
            $.ajax({
                type: 'PUT',
                url: '/home/settings/competencyalphas/update/' + id,
                data: {
