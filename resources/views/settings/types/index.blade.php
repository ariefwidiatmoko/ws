...
@section('button')
  ...
  <a href="{{ route('types.create') }}" class="btn btn-xs btn-success"><i class="fa fa-plus fa-fw"></i> New Type Score</a>
                    ...
                    <div class="btn-group" role="group">
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">
                        @can ('edit_positions', $item)
                          <a href="{{ route('types.edit', $item->id) }}" class="btn btn-xs btn-info">Edit</a>
                        @endcan
                      </div>
                      <div class="col-xs-1 margin" style="margin: -1px 8px -1px 8px;">
                        @can ('delete_positions', $item)
                          {!! Form::open( ['method' => 'delete', 'url' => route('types.destroy', $item->id), 'onSubmit' => 'return confirm("Are yous sure wanted to delete ' . $item->typename . ' ?")']) !!}
