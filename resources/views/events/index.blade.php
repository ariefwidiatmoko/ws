        ...
        <div class="row">
          <div class="col-md-6">
            <form class="" action="{{ route('events.store') }}" method="post">
              ...
              <div class="col-xs-3">
                <div class="form-group @if ($errors->has('event_start')) has-error @endif">
                  ...
                  <input value="{{ Carbon\Carbon::today()->format('Y-m-d') }}" name="event_start" type="text" class="form-control" id="datetimepicker1" data-date-format="YYYY-MM-DD" required>
                </div>
              </div>
              <!-- Event End -->
              <div class="col-xs-4">
                <div class="form-group @if ($errors->has('event_end')) has-error @endif">
                  ...
                  <div class="input-group">
                    <input value="{{ Carbon\Carbon::today()->format('Y-m-d') }}" name="event_end" type="text" class="form-control" id="datetimepicker2" data-date-format="YYYY-MM-DD" required>
                    <div class="input-group-btn">
            ...
            <table class="table table-hover table-bordered">
              <thead>
                <tr>
                  <th><i class="fa fa-circle fa-fw" style="color: black;"></i></th>
                  <th>Event Name</th>
              ...
              <tbody>
                @forelse ($table_events as $item)
                  <tr>
                    ...
                    <td onblur="update({{$item->id}})" id="event_end-{{$item->id}}" contenteditable class="text-center">
                      {{ $item->event_end->format('D, Y-M-d H:s') }}</td>
                    <td class="text-center">{{ ucfirst($item->user->name) }}</td>
                  </tr>
                @empty
                @endforelse
              </tbody>
