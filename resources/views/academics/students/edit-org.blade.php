...
@section('navmenu')
  ...
  <a href="{{ route('students.index') }}">Students</a> <i class="fa fa-angle-right fa-fw" style="color: #3c8dbc;"></i>
...
@include('academics.students._modal_add_year')
@endsection
...
@section('scripts')
    ...
    <script>
          ...
          $('.modal-footer').on('click', '.edit', function() {
              $.ajax({
                  type: 'PUT',
                  url: '/home/academics/students/update-allocate-student/' + id,
                  data: {
          ...
          $('.modal-footer').on('click', '.add', function() {
              $.ajax({
                  type: 'POST',
                  url: '/home/academics/students/add-year-student/' + id,
                  data: {
                      '_token': $('input[name=_token]').val(),
                      'id': $("#id_edit").val(),
                      'year_id': $('#year_edit').val(),
                      'classroom_id': $('#classroom_edit').val(),
                  },
                  success: function(data) {
                    if(data.message == 'Year successfully added.') {
                      window.location.reload();
